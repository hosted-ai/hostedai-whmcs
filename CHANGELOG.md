...
- VM / KVM instances are no longer billed at $0 — the API omits per-instance `total_cost` for VM-nature instances, so both crons fall back to `Helper::sumInstanceResourceCost()`, which sums all three per-interval cost buckets the API returns — **`Resources`** (CPU, RAM, GPU, vRAM, TFlops, disk, public IP, …), **`Services`** (service-policy charges), and **`pci_dev`** (PCI / GPU passthrough cards) — excluding the roll-up `total_cost`. This matches how the backend composes `total_cost`, so no dimension (incl. Service and GPU-card costs) is missed.
