...
- VM / KVM instances are no longer billed at $0 — the API omits per-instance `total_cost` for VM-nature instances, so both crons fall back to `Helper::sumInstanceResourceCost()`, which sums the interval `Resources` (CPU, RAM, GPU, vRAM, TFlops, disk, public IP, …) excluding the roll-up `total_cost`. Verified against the live API that `instance.total_cost` equals the `Resources` sum, so VM/KVM billing now matches how pods are billed.
- Per-instance itemized invoice descriptions — both crons now list the `Resources` cost breakdown per instance (data-driven from `Helper::instanceCostBreakdown()`), reconciling with the billed amount.
