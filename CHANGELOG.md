...
- **Hourly window no longer over-bills by one minute.** The billing API counts the requested window inclusive of both endpoints — a nominal *W*-minute window is billed as *W+1* minutes — so the prepaid cron's full 3600 s window billed 61 minutes and over-charged ~1.67 %/hour (e.g. €26.43 where the user panel shows €26.00 for the same hour). The window is now 3540 s (59 min) so the inclusive count lands on exactly 60 minutes, matching the panel; consecutive hourly runs tile cleanly (`04:24..05:23`, `05:24..06:23`) with no gap or double-billed boundary minute. Verified live: the old window returned 26.41, the new one 26.00.
