...
- VM / KVM instances are no longer billed at $0 — the API omits per-instance `total_cost` for VM-nature instances, so both crons fall back to `Helper::sumInstanceResourceCost()`, which sums **every** interval resource cost (CPU, RAM, **GPU**, disk, public IP, bandwidth, …) except the roll-up `total_cost`. Generic, so no cost dimension is missed.
